...
Vec3f barycentric(Vec2f p, Vec3f v0, Vec3f v1, Vec3f v2) {
...
void triangle(Model &m, TGAImage &image, TGAColor color) {
  ...
  //for (int i = 0; i < 1; ++i) {
    ...
    std::vector<float> x_extrema = {v0_n.x, v1_n.x, v2_n.x};
    std::vector<float> y_extrema = {v0_n.y, v1_n.y, v2_n.y};
    ...
    for (int x = x_min; x < x_max; ++x) {
      for (int y = y_min; y < y_max; ++y) {
        Vec2f point(x, y);
        //image.set(x, y, WHITE);
        Vec3f u_v_1 = barycentric(point, v0_n, v1_n, v2_n);
        if (!(u_v_1.x < 0 || u_v_1.y < 0 || u_v_1.z < 0) && u_v_1.z >= 1 ) {
          image.set(x, y, color);
        }
      }
    }
